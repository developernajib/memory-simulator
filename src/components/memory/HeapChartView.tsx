...
export function HeapChartView() {
  ...
  const taskB = rows.filter((r) => r.name?.includes('Task') && !r.name.includes('Manager')).length * 120;
  ...
  return (
    <div style={{ padding: 12, flex: 1, overflowY: 'auto' }}>
      <div style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--t2)', marginBottom: 12 }}>
      ...
      >
        <div style={{ width: pct(rtB), background: 'var(--purple)', transition: 'width .4s' }} title="Runtime" />
        <div style={{ width: pct(taskB), background: 'var(--red)', transition: 'width .4s' }} title="Task data" />
        <div style={{ width: pct(strB), background: 'var(--yellow)', transition: 'width .4s' }} title="Strings" />
        <div style={{ width: pct(chanB), background: 'var(--cyan)', transition: 'width .4s' }} title="Channels" />
        <div style={{ flex: 1, background: 'var(--b3)' }} title="Free" />
      </div>
      <SegmentBar val={rtB} label="Runtime (G descriptors, stacks)" color="var(--purple)" total={total} />
      <SegmentBar val={taskB} label="Task structs + backing arrays" color="var(--red)" total={total} />
      ...
      <div
        style={{ marginTop: 16, fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--t3)', lineHeight: 1.8 }}
      >
